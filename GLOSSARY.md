...
| RD Engine doctrine spine | RD Engine doctrine | ~ | ~ | Downstream doctrine / architectural lineage | The broader reducer-governed architectural doctrine this app follows, even though the implementation here is Flutter rather than the Rust `rd-engine` crate | Not a claim that the app literally uses the `rd-engine` crate; not generic mobile MVC language | chatty-pet/README.md |
